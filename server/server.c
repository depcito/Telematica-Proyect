...
static const char *unit_for_type(const char *type) {
    if (strcmp(type, TYPE_TEMPERATURE) == 0) return UNIT_CELSIUS;
    if (strcmp(type, TYPE_VIBRATION)   == 0) return UNIT_MM_S;
    if (strcmp(type, TYPE_ENERGY)      == 0) return UNIT_WATTS;
    if (strcmp(type, TYPE_HUMIDITY)    == 0) return UNIT_PERCENT;
    if (strcmp(type, TYPE_STATUS)      == 0) return UNIT_BINARY;
    return "";
}
...
static int validate_token(const char *token, char *out_username, size_t out_size) {
    ...
    const char *auth_host = "auth-server";  /* nombre DNS / hostname */
    int         auth_port = 5001;
...
static void handle_register_operator(int fd, const char *client_ip, int client_port,
    ...
    int slot = -1;
    for (int i = 0; i < state.operator_count; i++) {
        if (!state.operators[i].active) { slot = i; break; }
    }
    if (slot == -1) {
...
static void handle_data(int fd, const char *client_ip, int client_port,
    ...
    if (s->history_count < MAX_SENSOR_HISTORY) s->history_count++;
...
static void handle_get_data(int fd, const char *client_ip, int client_port,
    ...
    if (n > MAX_SENSOR_HISTORY) n = MAX_SENSOR_HISTORY;
    ...
    for (int i = available - 1; i >= 0; i--) {
        int idx = (s->history_index - 1 - i + MAX_SENSOR_HISTORY)
                  % MAX_SENSOR_HISTORY;
        offset += snprintf(resp + offset, sizeof(resp) - offset,
                           " %.2f:%ld",
                           s->history[idx].value,
...
static void handle_get_status(int fd, const char *client_ip, int client_port) {
    ...
    int  operators = 0;
    for (int i = 0; i < state.operator_count; i++)
        if (state.operators[i].active) operators++;
...
static void *client_thread(void *arg) {
    ...
    while (1) {
        ...
        if (n <= 0) break;   /* cliente cerró o error de red */
        buf[n] = '\0';
